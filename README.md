...
| **Tandem duplications** | ⚠️ Inherent | Tandem duplications produce alignment blocks that overlap heavily in reference space.  Both blocks are returned as `"alignment"` events and correctly extracted as sequence, but no `event_type` gap is emitted because the reference interval is fully covered.  Callers that need explicit tandem-duplication annotations should detect overlapping `"alignment"` results mapping to different assembly coordinates |
| **CIGAR insertion boundaries** | ⚠️ Safeguarded | An assembly insertion mapped exactly at a query boundary is invisible to `project_cigar()` because no reference bases are consumed at the insertion site.  `compute_regions()` in `generate_toy_dataset.py` enforces a minimum padding of 100 bp (`_MIN_PADDING`) so that breakpoint-adjacent insertions always fall inside the queried window.  Users writing custom queries should include sufficient padding around variant breakpoints |
